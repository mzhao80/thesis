...
class WikiSingleClassifier(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes, dropout_rate=0.1, num_layers=4):
        """
        Classifier for wiki-single approach (single encoder for document, topic, and wiki).
        """
        super(WikiSingleClassifier, self).__init__()
        self.input_norm = nn.RMSNorm(input_dim)
        self.input_proj = nn.Linear(input_dim, hidden_dim * 2)
        self.layers = nn.ModuleList()
        for _ in range(num_layers):
...
class WikiDualClassifier(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes, dropout_rate=0.1, num_layers=4):
        """
        Classifier for wiki-dual approach (separate encoders for document-topic and wiki).
        """
        super(WikiDualClassifier, self).__init__()
        self.input_norm = nn.RMSNorm(input_dim)
        self.input_proj = nn.Linear(input_dim, hidden_dim * 2)
        self.layers = nn.ModuleList()
        for _ in range(num_layers):
