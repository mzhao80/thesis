...
class VASTZeroFewShot(Dataset):
    def __init__(self, phase, model='bert-base-uncased', wiki_model='', inference_mode=False):
        if not inference_mode:
            ...
            # Inference mode - use training_data.csv format
            df = pd.read_csv('/n/holylabs/LABS/arielpro_lab/Lab/michaelzhao/training_data.csv')
            print(f'# of inference examples: {df.shape[0]}')
            df = df.dropna(subset=['policy_area', 'document', 'summary'])
            print(f'# of inference examples after dropping na: {df.shape[0]}')

            topics = df['policy_area'].tolist()  # Use policy_area as topic
            tweets = df['document'].tolist()  # Use document as text
            stances = [-1] * len(tweets)  # No labels in inference mode
            few_shot = np.zeros(df.shape[0])
            qte = np.zeros(df.shape[0])
            sarc = np.zeros(df.shape[0])
            imp = np.zeros(df.shape[0])
            ...
            # Use summary as wiki text for inference
            wiki_summaries = df['summary'].tolist()
        ...
        self.original_data = df  # Store original data for output
...
def data_loader(data, phase, topic, batch_size, model='bert-base-uncased', wiki_model='', n_workers=4, inference_mode=False):
    shuffle = True if phase == 'train' else False
    dataset = VASTZeroFewShot(phase, model=model, wiki_model=wiki_model, inference_mode=inference_mode)
