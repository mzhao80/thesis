#!/usr/bin/env python
"""
Mean Embedding Utilities for Taxonomy Expansion

This module provides utilities to work with mean embeddings of documents for clustering
and classification tasks in the taxonomy expansion pipeline. It includes functions
for embedding correction, visualization, and cluster validity assessment.

Key features:
- Enforcing majority cluster assignment for identical embeddings
- Adding "Misc." embeddings for unclassified documents
- Computing optimum clustering parameters
- Generating cluster labels using OpenAI APIs
- Visualizing embeddings and clusters with UMAP
"""
...
def enforce_majority_cluster(embeddings, cluster_labels, misc_embedding):
    ...
    This function identifies duplicated embeddings and ensures they are consistently 
    assigned to the same cluster based on majority vote.
    
    Args:
        embeddings (np.ndarray): Array of document embeddings
        cluster_labels (np.ndarray): Array of cluster labels for each embedding
        misc_embedding (np.ndarray): Embedding vector for the "Misc." class
        
    Returns:
        np.ndarray: Corrected cluster labels with consistent assignments
    """
...
def compute_mean_embedding_text(texts, embeddings):
    """
    Given a list of texts and their embeddings, compute the mean embedding
    and find the text that is closest to this mean embedding.
    ...
    Returns:
        str: Text closest to the mean embedding
    """
    # Calculate mean embedding
    mean_embedding = np.mean(embeddings, axis=0, keepdims=True)
    
    # Find closest existing text to mean embedding
...
def generate_cluster_label(subtopics, parent_topic, client, max_attempts=5):
    ...
    Args:
        ...
        max_attempts (int): Maximum number of attempts to generate a label
    ...
    system_message = (
        ...
        "When given a first-level parent topic and a list of topics of documents clustered under the parent topic, you must generate a concise and representative second-level topic (2-5 words) "
    ...
    prompt = (
        ...
        "the common topic the document topics represent. It must be a policy controversy one can take a stance on (for or against). It should be broad enough to cover ALL the clustered document topics. Output the topic with an inherent stance, such as Expanding Refugee Visa Programs instead of Refugee Visa Policy or Refugee Controversy. Avoid using the general words 'reform' and 'controversy'. You must give a direction (increasing, decreasing, expanding, cutting, etc.), e.g. Expanding Voting Rights rather than Voting Reform. \n\n"
...
def main():
    """
    Main function to execute the taxonomy expansion pipeline.
    
    This function orchestrates the entire pipeline, including data loading, 
    embedding computation, clustering, and visualization.
    """
    n_components = 10
    n_neighbors = 25
    min_cluster_size = 25
    ...
    for _, row in df.iterrows():
        ...
        if path not in source_mapping:
            source_mapping[path] = []
        source_mapping[path].append(row['matching_idx'])
    ...
    for subtopic in tqdm(df['pred_subtopic'].unique(), desc="Generating embeddings"):
        ...
        # normalize embedding - convert to torch tensor first, then back to numpy
    ...
    # Also normalize the Misc. embedding
    ...
    for parent_topic in tqdm(df['policy_area'].unique(), desc="Clustering"):
        ...
        if len(subtopics) < n_components + 2:
            ...
            taxonomy.append({
                ...
                'mean_embedding_text': "Misc.",  # Added mean embedding text
                'gpt_label': "Misc.",
                'pred_subtopics': "Misc.",
        ...
        for label, texts in clusters.items():
            ...
            if label != -1:
                ...
                mean_embedding_label = compute_mean_embedding_text(np.array(texts), cluster_original_embeddings)  # Added mean embedding computation
                gpt_label = generate_cluster_label(texts, parent_topic, client)
            else:
                new_topic_label = "Misc."
                new_reduced_topic_label = "Misc."
                mean_embedding_label = "Misc."  # Added for miscellaneous cluster
                gpt_label = "Misc."
            ...
            taxonomy.append({
                ...
                'mean_embedding_text': mean_embedding_label,  # Added mean embedding text
                'gpt_label': gpt_label,
    ...
    # Write taxonomy to original CSV and new embedding_check.csv
    taxonomy_df = pd.DataFrame(taxonomy)
    taxonomy_df.to_csv(f'embedding_check.csv', index=False)
    
    print(f"\nTaxonomy saved to embedding_check.csv with weighted DBCV score: {weighted_score:.3f}")
