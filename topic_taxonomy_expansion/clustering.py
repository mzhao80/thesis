...
This script performs clustering on the first-level subtopics generated by topic_generation.py.
It uses HDBSCAN with UMAP dimensionality reduction to group similar subtopics together and 
then generates representative labels for each cluster using GPT-4o-mini.

The script:
1. Loads predicted subtopics from step_1.csv
...
6. Outputs the clustering results to step_2.csv

Output:
- step_2.csv: Contains policy areas, cluster labels, and source indices
- Visualization plots in the cluster_viz directory
"""
...
def generate_cluster_label(subtopics, parent_topic, client, max_attempts=5):
    ...
    Args:
        ...
        parent_topic (str): The parent topic (policy area)
...
def main():
    """
    Main function to perform clustering on the first-level subtopics.
    """
    # UMAP and HDBSCAN parameters
    n_components = 10         # Dimensionality for UMAP reduction
    n_neighbors = 25          # Number of neighbors for UMAP
    min_cluster_size = 25     # Minimum cluster size for HDBSCAN
    ...
    print("Loading data from step_1.csv...")
    ...
    # Create source mapping (policy_area, pred_subtopic) -> list of matching_idx
    source_mapping = {}
    ...
    # Process each policy area separately
    for parent_topic in tqdm(df['policy_area'].unique(), desc="Clustering"):
        ...
        # Get unique predicted subtopics for this parent topic
        subtopics = sub_df['pred_subtopic'].tolist()
        print(f"Total subtopics: {len(subtopics)}")
        
        # Skip if too few subtopics for meaningful clustering
        ...
        plt.title(f"UMAP projection of {parent_topic} subtopics (Clusters: {num_clusters})")
        plt.savefig(f"cluster_viz/{parent_topic.replace('/', '_')}_umap.png")
        plt.close()
        ...
        for label in tqdm(sorted(list(set(cluster_labels))), desc="Processing clusters"):
            if label == -1:
                ...
                taxonomy.append({
                    ...
                    'medoid_text': "Misc. (Noise)",
                    'reduced_medoid_text': "Misc. (Noise)",
                    'gpt_label': "Misc.",
                    'pred_subtopics': "; ".join(noise_texts),
                    'source_indices': ";".join(source_indices)
                })
                continue
            ...
            cluster_embeddings_original = np.array([original_embeddings[i] for i in indices])
            cluster_embeddings_reduced = np.array([reduced_embeddings[i] for i in indices])
            
            # Compute medoids
            medoid_text = compute_medoid(cluster_texts, cluster_embeddings_original)
            reduced_medoid_text = compute_medoid(cluster_texts, cluster_embeddings_reduced)
            ...
            taxonomy.append({
                ...
                'reduced_medoid_text': reduced_medoid_text,
                'gpt_label': gpt_label,
                'pred_subtopics': "; ".join(cluster_texts),
                'source_indices': ";".join(source_indices)
            })
    ...
    print("Taxonomy saved to step_2.csv")
