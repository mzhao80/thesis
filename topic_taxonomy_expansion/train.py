...
def build_core_graph(topic_list, device):
    """
    Build core graphs for downward, upward, and sideward relations.
    Graph structure:
      - Node 0: "Political Issue" (parent)
      - Nodes 1..N-1: 32 core topics.
    Edges:
      - Downward: from 0 to each core topic.
      - Upward: from each core topic to 0.
...
def main(config):
    ...
    # Create a filtered state dictionary that excludes the document encoder parameters.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train Topic Expansion Model with learnable fusion, configurable loss weighting, and gradient clipping")
